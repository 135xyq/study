...
router.beforeEach((to, from, next) => {
    ...
    if (to.meta.auth) {
        if (store.state.user != null) {
            ...
            if (!store.state.isAdded) {
                const changeRouters = getResultRouter(store.state.user.role, changeRouterMap);
                // console.log(2)
                store.dispatch('changeRouterMenus', routes.concat(changeRouters)).then(() => {
