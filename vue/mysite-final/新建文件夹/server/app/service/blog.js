...
class BlogService extends Service {
    ...
    async increaseScanNumber(id) {
        await this.ctx.model.Blog.updateOne({ _id: id }, { $inc: { scanNumber: 1 } });
    }
    ...
    async add(info) {
        ...
        await this.ctx.model.BlogType.updateOne({ _id: obj.categoryId }, { $inc: { articleCount: 1 } });
    ...
    async update(id, info) {
        ...
        if (blog.categoryId) {
            // 之前的类别文章数量-1
            await this.ctx.model.BlogType.updateOne({ _id: blog.categoryId }, { $inc: { articleCount: -1 } });
        }
        // 当前类别的文章数量+1
        await this.ctx.model.BlogType.updateOne({ _id: info.categoryId }, { $inc: { articleCount: 1 } });
