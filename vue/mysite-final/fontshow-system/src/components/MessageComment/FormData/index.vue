    ...
    <div class="nickcontent">
      <div class="textarea">
        <textarea
          maxlength="300"
          placeholder="请填写内容"
          v-model="formData.nickcontent"
        ></textarea>
        <span class="tip">{{ `${formData.nickcontent.length}/300` }}</span>
      </div>
...
export default {
  ...
  methods: {
    handleSubmit() {
      this.error.nickname = this.formData.nickname.trim() ? "" : "请填写昵称!";
      this.error.nickcontent = this.formData.nickcontent.trim() ? "" : "请输入要评论内容!";
